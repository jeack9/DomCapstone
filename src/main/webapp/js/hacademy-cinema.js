(function (w) {
    ...
    w.Hacademy.Reservation = function (area, callback) {
        ...
        var seatheight = area.querySelector(".cinema-seat:not(.empty)");
        if (!seatArea) {
        ...
        var seatAreaHeight = calculateInnerHeight(seatheight);
        ...
        //전송 이름(없으면 seat으로 설정)
        var sendName = area.dataset.name || '';
    ...
    w.Hacademy.Reservation.prototype.getQueryString = function () {
        ...
        for (var i = 0; i < checkedList.length; i++) {
            ...
            //unit.setAttribute("id", this.options.value);
            params.append(unit.name, unit.value);
            //params.append(name, unit.value);
        }
        return params.toString();
    };

    w.Hacademy.Reservation.prototype.setDisable = function () {
        var activeSeat = this.options.seatArea.querySelector(".cinema-seat.active");
    ...
    w.Hacademy.Reservation.prototype.createUnit = function (v) {
        ...
        if (v === 'empty') {
            ...
            if (this.options.seatNoVisible === "visible") {
                ...
                seat.setFillRoom = function (v) {
                    people.textContent = v + "/" + 2;
                }
                ...
                people.addEventListener("contextmenu", function (e) {
                    e.preventDefault();
                    e.stopPropagation();
                    seat.dispatchEvent(new Event("contextmenu"));
                });
    ...
    w.Hacademy.Reservation.prototype.changeUnit = function (unit, mode) {
        var app = this;
        var dataHosu = unit.getAttribute('data-hosu');
        if (mode === "space") {
    ...
    w.Hacademy.Reservation.prototype.setValue = function (unit) {
        ...
        var value = "";
        // if (unit.dataset.row == 3) {
        //     if (unit.dataset.col >= 10) {
        //         value += 1;
        //         value += (parseInt(unit.dataset.col) + parseInt(15));
        //     } else {
        //         value += 1;
        //         value += (parseInt(unit.dataset.col) + parseInt(16)).toString();
        //     }
        ...
        //         value += seperator;
        //         value += unit.dataset.col - 1;
        //     }
        //     else if (unit.dataset.col > 10) {
        //         value += unit.dataset.row;
        //         value += unit.dataset.col - 1;
        //     }
        //     else {
        ...
        unit.querySelector("input").id = value;
        unit.querySelector("input").value = value;
        unit.querySelector("input").name = value;;
    }
    ...
    w.Hacademy.Reservation.prototype.resize = function () {
        ...
        for (var i = 0; i < seatList.length; i++) {
            ...
            // seatList[i].style.height = "140px";
            seatList[i].style.height = (this.options.unitSize * 2) + "px";
        }
    ...
    function calculateInnerWidth(tag) {
        ...
        var fix = 17;//17px만큼 오차가 발생하는 원인을 아직 파악하지 못함
    ...
    function calculateInnerHeight(tag) {
        var style = window.getComputedStyle(tag, null);
        var fix = 17;//17px만큼 오차가 발생하는 원인을 아직 파악하지 못함
        if (style.boxSizing === 'border-box') {
            console.log(style.height);
            return parseFloat(style.height) - fix;
    ...
    w.Hacademy.Reservation.prototype.changeroom = function () {
